...
interface ControlButtons {
  ...
  prev10min: HTMLButtonElement;
  prev30sec: HTMLButtonElement;
  next10min: HTMLButtonElement;
  next30sec: HTMLButtonElement;
}
...
class Videos {
  ...
  private controls!: ControlButtons;
  private progressBar!: HTMLElement;
  private isPlaying: boolean = false;
  private animationId: number | null = null;
  private skipTime: number = 20; // seconds for tab skip
  ...
  private createVideoElement(): void {
    ...
    this.video.style.boxShadow = '0 4px 20px rgba(0, 0, 0, 0.3)';
  ...
  private bindEvents(): void {
    ...
    document.addEventListener('keydown', (e) => {
      if (e.key === 'Tab') {
        e.preventDefault();
        if (e.shiftKey) {
          this.skip(-this.skipTime); // Shift + Tab: rewind 20s
        } else {
          this.skip(this.skipTime); // Tab: forward 20s
        }
      }
    });
  }
  ...
  private animatePauseButton(): void {
  ...
  private showSkipAnimation(direction: 'forward' | 'backward'): void {
